...
(function () {
  ...
  ns.NoiseTool = function () {
    ...
    this.helpText = 'Darker noise tool';
  ...
  ns.NoiseTool.prototype.applyToolOnPixel = function (col, row, frame, overlay, event) {
    ...
    var noiseColor = this.pSBC(color, pskl.app.selectedColorsService.getPrimaryColor(), pskl.app.selectedColorsService.getSecondaryColor(), true);
    this.draw(noiseColor, col, row, frame, overlay);
  };

  ns.NoiseTool.prototype.pSBCr = function (d) {
    let i = parseInt;
    let m = Math.round;
    let n = d.length, x = {};
    if (n > 9) {
  ...
  ns.NoiseTool.prototype.pSBC = function (p, c0, c1, l) {
